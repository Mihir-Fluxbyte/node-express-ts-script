...
# npx json -I -f package.json -e "this.scripts={...this.scripts, 'build':'tsc', 'start':'tsc && node dist/index.js','dev':'nodemon --watch \"src/**/*.ts\" --exec \"ts-node\" src/index.ts','lint':'eslint .','lint:fix':'eslint . --fix','format':'prettier --ignore-path .gitignore --write \"**/*.+(js|ts|json)\"'}"
